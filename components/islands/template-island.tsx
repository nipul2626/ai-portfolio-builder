...
export function TemplateIsland({ onTemplateSelect, onBack }: TemplateIslandProps) {
  ...
  const [direction, setDirection] = useState(0) // <-- moved to top level
  ...
  // make index safe (wrap-around) in case filteredTemplates length changes:
  const safeIndex = filteredTemplates.length
      ? ((currentIndex % filteredTemplates.length) + filteredTemplates.length) % filteredTemplates.length
      : 0

  const currentTemplate = filteredTemplates[safeIndex]
          ...
          <div className="relative perspective-1000">
            <div className="relative h-[500px] flex items-center justify-center">
              ...
              {[-2, -1, 1, 2].map((offset) => {
                ...
                const index = (safeIndex + offset + filteredTemplates.length) % filteredTemplates.length
              ...
              <div className="relative w-96 h-[450px]">
                <AnimatePresence initial={false} custom={direction}>
                  {currentTemplate && (
                      <motion.div
                          key={currentTemplate.id}               // keyed so AnimatePresence animates exit/enter
                          custom={direction}
                          ...
                          className="absolute inset-0 rounded-3xl overflow-hidden border-2 border-cyan-500/50 shadow-2xl shadow-cyan-500/20 transform-3d"
                      >
                        {/* Template preview */}
